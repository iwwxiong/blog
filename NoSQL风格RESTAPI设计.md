...
`GET /users?select=id,age,name&age=gte.25&order=id.desc&limit=10  HTTP / 1.1`，查询前10个姓名大于等于25的用户，返回用户的id, age, name且按照id倒叙排列
...
`GET /users?age=lt.25 HTTP/1.1`， 查询年龄小于25的用户

多个条件组合：

`GET /users?age=lte.25&name=like.xxx`，查询年龄小于等于25且姓名like xxx的用户
...
`GET /users?select=id,name,school{*}&school.name=xxx`，查询学校名称为xxx的用户，且返回用户的id,name和学校的所有字段
...
`PUT /books?name='Python'`，批量更新名称为Python的书籍，变更作者id为2
...
`DELETE /books?user.name='xxx'`，删除作者名称为xxx的所有书籍

## 注意这种风格的RESTAPI可能会造成部分问题。比如恶意的分页数量，恶意的连表查询造成后端性能等问题。此时就需要后端也牺牲一些自由行，做部分字段管控等。
